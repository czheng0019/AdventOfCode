...
for line in arr:
    ...
    if list(occDict.keys()).count("J") >= 1:
        jCount = occDict.get("J")
        if jCount != 5:
            occDict.pop("J")
            sortedList = sorted(occDict.items(), key=lambda x: x[1], reverse=True)
            occDict = dict(sortedList)
            occDict.update({list(occDict.keys())[0]: occDict.get(list(occDict.keys())[0]) + jCount})
